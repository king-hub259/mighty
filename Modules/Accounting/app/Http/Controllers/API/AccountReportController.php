...
class AccountReportController extends Controller
{
    ...
    {
        $year = (int) $request->year ?? current_year();
        ...
        // Fetch cash flow data for the given year
        $cashFlowStatement = DB::table('account_transactions')
            ...
            ->get()
            ->keyBy('month'); // Key by month to easily merge

        // Prepare the result array with all months and set 0 if no data exists
        $result = [];

        foreach ($months as $month) {
            $monthData = $cashFlowStatement->get($month);

            // If no data exists for the month, set the values to 0
            if ($monthData) {
                $result[] = [
                    'year' => $monthData->year,
                    'month' => $month,
                    'total_debit' => $monthData->total_debit,
                    'total_credit' => $monthData->total_credit,
                ];
            } else {
                $result[] = [
                    'year' => $year,
