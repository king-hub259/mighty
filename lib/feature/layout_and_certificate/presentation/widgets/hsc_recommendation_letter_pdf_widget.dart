...
class HscRecommendationLetterPdfPreviewPage extends StatelessWidget {
  const HscRecommendationLetterPdfPreviewPage({super.key});
  ...
  Future<Uint8List> _generatePdf(PdfPageFormat format) async {
    ...
    pdf.addPage(pw.Page(pageFormat: format, margin: const pw.EdgeInsets.all(32), build: (pw.Context context) {
      return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.stretch, children: [
        ...
        pw.RichText(text: pw.TextSpan(children: [
          ...
          pw.TextSpan(text: data?.studentSession?.student?.fatherName ?? 'N/A', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
          const pw.TextSpan(text: ' and '),
          pw.TextSpan(text: data?.studentSession?.student?.motherName ?? 'N/A', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
          const pw.TextSpan(text: ' was a student of '),
          pw.TextSpan(text: institute?.schoolName ?? 'N/A', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Preview HSC Recommendation Letter PDF")),
      body: PdfPreview(
