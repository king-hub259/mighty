...
class EmployeeDropdown extends StatefulWidget {
  ...
  final Function(EmployeeItem?)? onChanged;
  final EmployeeItem? selectedValue;
  const EmployeeDropdown({super.key, this.width, this.border = false, required this.title, this.items, this.onChanged, this.selectedValue});
...
class _EmployeeDropdownState extends State<EmployeeDropdown> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(decoration: BoxDecoration(
      ...
      child: DropdownButton2<EmployeeItem>(
        isExpanded: true,
        ...
        widget.items?.map((EmployeeItem item) => DropdownMenuItem<EmployeeItem>(
